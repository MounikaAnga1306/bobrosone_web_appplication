...
import { FaSearch, FaExchangeAlt, FaCalendarAlt, FaUser, FaMapMarkerAlt, FaChevronDown, FaTimes, FaPlane, FaSpinner } from 'react-icons/fa';

const FlightSearchScreen = () => {
  const navigate = useNavigate();
  const { updateSearchParams, updateFlightResults, flightResults } = useFlightSearchContext();
  ...
  const updateTravellers = (type, action) => {
    setTravellers(prev => {
      ...
      if (action === 'increment') {
        ...
        if ((type === 'children' || type === 'infants') && currentValue <= 0) return prev;
        newValue = currentValue - 1;
      }
  ...
  return (
    <div className="w-full bg-gray-50">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <p className="text-white/90 text-base md:text-lg drop-shadow-xl max-w-2xl">
                Book flights effortlessly with the best prices and comfort
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 mb-8">
        {/* Flight Booking Card */}
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-8 border border-gray-200">
          <h1 className="text-2xl font-bold text-gray-800 mb-6">Flight Booking</h1>
          
          {/* Trip Type Toggle */}
          <div className="flex items-center mb-6">
            <div className="flex items-center bg-gray-100 rounded-lg p-1 border border-gray-200">
              <button
                className={`px-5 py-2 text-sm font-medium rounded-md transition-all duration-200 ${tripType === 'one-way' 
              ...
              <button
                className={`px-5 py-2 text-sm font-medium rounded-md transition-all duration-200 ${tripType === 'round-trip' 
                  ? 'bg-white text-gray-900 shadow-sm border border-gray-300' 
                  : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4">
                  ...
                  {showFromDropdown && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-200 z-50 max-h-64 overflow-y-auto">
                      <div className="p-3">
                        ...
                        ) : fromAirports.length === 0 && fromDisplay.length >= 3 ? (
                          <div className="text-center py-4 text-gray-500">
                            No airports found
                          </div>
                        ) : (
              ...
              <div className="lg:col-span-3 relative" ref={toRef}>
                ...
                <div className="relative">
                  {/* Swap Button */}
                  <button 
                    className="absolute -left-4 top-1/2 transform -translate-y-1/2 z-10 bg-white border border-gray-300 rounded-full p-2 hover:bg-gray-50 transition-colors shadow-sm"
              ...
              <div className="lg:col-span-2">
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">Departure</label>
                <div className="relative">
              ...
              <div className="lg:col-span-2 relative" ref={travellerRef}>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">Travellers & Class</label>
                <div className="relative">
                  <div 
                  ...
                  >
                    ...
                    <FaChevronDown className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 transition-all duration-200 ${showTravellerModal ? 'rotate-180' : ''}`} />
                  </div>
                  ...
                  {showTravellerModal && (
                    <div className="absolute top-full left-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 p-5">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-gray-800">Travellers & Class</h3>
                        <button 
                      ...
                      <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-700">Max {maxTravellers} travellers allowed</span>
                          <span className={`text-sm font-bold ${totalTravellers >= maxTravellers ? 'text-red-600' : 'text-green-600'}`}>
                      ...
                      <div className="space-y-4 mb-6">
                        {[
                          ...
                          { type: 'children', label: 'Children (2-12 yrs)', min: 0 },
                          { type: 'infants', label: 'Infants (Below 2 yrs)', min: 0 }
                        ].map(({ type, label, min }) => (
                          <div key={type} className="flex justify-between items-center">
                            <div>
                              <div className="font-medium text-gray-800">{label}</div>
                              {type === 'infants' && (
                                <div className="text-xs text-gray-500 mt-1">1 infant per adult (auto-adjusted)</div>
                              )}
                            </div>
                            <div className="flex items-center space-x-3">
                              <button
                                ...
                                onClick={() => updateTravellers(type, 'decrement')}
                                disabled={travellers[type] <= min}
                              >
                                <span>−</span>
                              </button>
                              <span className="font-bold text-gray-800 min-w-[20px] text-center">{travellers[type]}</span>
                              <button
                                ...
                                onClick={() => updateTravellers(type, 'increment')}
                                disabled={totalTravellers >= maxTravellers || type === 'infants'}
                              >
                                <span>+</span>
                              </button>
                            </div>
                      ...
                      {/* Class Selection */}
                      <div className="mb-6">
                        <h4 className="font-medium text-gray-800 mb-3">Travel Class</h4>
                        <div className="grid grid-cols-2 gap-2">
                          {travelClasses.map((travelClass) => (
                            <button
                              key={travelClass}
                              className={`py-2.5 px-3 rounded-lg text-sm font-medium transition-all ${travellers.class === travelClass 
              ...
              {/* SEARCH BUTTON */}
              <div className={`${tripType === 'one-way' ? 'lg:col-span-2' : 'lg:col-span-2'}`}>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide opacity-0">Search</label>
                <button
                  onClick={handleSearch}
                  disabled={flightResults.loading || !selectedFromAirport || !selectedToAirport}
                  className="w-full bg-gradient-to-r from-[#FD561E] to-[#ff7b4a] text-white py-3.5 px-4 rounded-lg font-bold text-lg hover:from-[#e54d1a] hover:to-[#ff6a3c] transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center space-x-2 transform hover:scale-[1.02] h-[52px] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                >
            ...
            <div className="pt-6 border-t border-gray-200">
              <h3 className="text-sm font-medium text-gray-700 mb-4">Special Fares (Optional):</h3>
              <div className="flex gap-3">
                {['Student', 'Senior Citizen', 'Armed Forces'].map((fare) => (
                  <button
                    key={fare}
                    className="flex flex-col items-center px-4 py-2 border border-gray-300 rounded-lg hover:border-[#FD561E] transition-all duration-200 group"
                  >
                    <span className="font-semibold text-gray-800 group-hover:text-[#FD561E]">{fare}</span>
                    <span className="text-xs text-gray-600">
                      {fare === 'Student' ? 'Upto 10% off' : 
                       fare === 'Senior Citizen' ? 'Upto 15% off' : 
                       'Special fares'}
                    </span>
                  </button>
                ))}
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <h2 className="text-xl font-bold text-gray-800 mb-6">Do More With BOBROS</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
            {['Flight Tracker', 'Credit Card', 'Cruise', 'Book Visa', 'Group Booking', 'Plan', 'Fare Alerts'].map((item, index) => (
              <div 
                key={index} 
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700 text-center group-hover:text-[#FD561E]">{item}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <h2 className="text-xl font-bold text-gray-800 mb-6">Today's Flight Offers</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { route: 'Delhi → Mumbai', price: '₹2,499', airline: 'IndiGo', duration: '1h 30m' },
              { route: 'Bangalore → Delhi', price: '₹3,199', airline: 'Air India', duration: '2h 30m' },
              { route: 'Mumbai → Goa', price: '₹1,899', airline: 'SpiceJet', duration: '1h 15m' },
              { route: 'Chennai → Kolkata', price: '₹2,899', airline: 'Vistara', duration: '2h 15m' }
            ].map((offer, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all duration-300 hover:border-[#FD561E] group">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-bold text-gray-800 group-hover:text-[#FD561E]">{offer.route}</h3>
                    <div className="flex items-center mt-1 space-x-2">
                      <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{offer.duration}</span>
                      <span className="text-xs text-gray-500">{offer.airline}</span>
                    </div>
                  </div>
                  <span className="text-[#FD561E] font-bold text-lg">{offer.price}</span>
                </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="space-y-6">
            <div className="p-4 hover:bg-gray-50 rounded-lg transition-colors duration-200">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Why is flight ticket booking the cheapest on BOBROS?</h3>
              <p className="text-gray-600">BOBROS directly searches multiple airline websites for the cheapest fares. Many airlines sell their cheapest flight tickets on BOBROS. Additionally, with its exclusive offers and deals, including several bank and partner offers, BOBROS serves as the best and cheap platform to book cheap flights online.</p>
            </div>
          </div>
        </div>
      </div>
