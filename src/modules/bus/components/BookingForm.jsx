...
const BookingForm = () => {
  ...
  // validIndexes: load ayye images matrame (broken images skip avtayi).
  // ──────────────────────────────────────────────────────────────
  ...
  // Anni background images ni mundu ga preload chestam (cache).
  useEffect(() => {
  ...
  // ──────────────────────────────────────────────────────────────
  // Crossfade transition — LOAD-GATED.
  // Target image ni mundu load chesi, load aithe ne crossfade start chestam.
  // Image broken/missing aithe (eg. bluecolor.png) → aa slide ni skip chesi
  // next image ki veltam. So gray/white blank slide eppudu kanipinchadu.
  // ──────────────────────────────────────────────────────────────
  const transitionTo = (newIndex, attempts = 0) => {
    const len = backgroundImages.length;
    if (newIndex === currentIndexRef.current) return;
    if (attempts >= len) return; // anni broken aithe — loop avoid
    ...
    const skipBroken = () => {
      ...
      currentIndexRef.current = newIndex; // ee index daatesindi ani mark
    ...
    // already cached aithe complete + naturalWidth check chesi sync ga handle
    if (img.complete) {
  ...
  return (
    <section className="relative min-h-[550px] md:min-h-[480px] lg:min-h-[590px] flex items-center justify-center py-8 md:py-0 overflow-hidden">
      ...
      {/* Background Carousel — base layer (always opaque) + crossfade layer on top.
          Wrapper ki fallback dark color => edaina image fail aina white/gray badulu
          dark color kanipistundi (transparent gap eppudu raadu). */}
      <div className="absolute inset-0 w-full h-full" style={{ backgroundColor: "#0f172a" }}>
