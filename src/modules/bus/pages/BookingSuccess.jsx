  ...
  const handleRazorPayClick = async (fareToCharge) => {
    ...
    try {
      ...
      rzp.on("payment.failed", function (r) {
        navigate("/payment-status", {
          state: { status: "failed", payment: { code: "PAYMENT_ERROR", reason: r.error.message } }
        });
      });
      rzp.open();
    } catch (error) {
      navigate("/payment-status", {
        state: { status: "failed", payment: { code: "PAYMENT_ERROR", reason: error.message } }
      });
    }
  };
        ...
        >
          ...
          <div>
            ...
            {isGuest && (
              <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8e8e8", padding: "20px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: "12.5px", color: "#666", marginBottom: "16px", lineHeight: "1.5" }}>
                  Sign up with BOBROS to avail great discounts and earn reward points.
                </div>
                <button
                  ...
                  style={{
                    ...
                    fontWeight: "700",
                    cursor: "pointer",
                    boxShadow: "0 3px 10px rgba(253,86,30,0.3)"
                  }}
                >
                  Sign Up
                </button>
          ...
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8e8e8", padding: "18px" }}>
              ...
              {passengers.map((p, i) => (
                <div key={i} style={{
                  marginBottom: i < passengers.length - 1 ? "14px" : "0",
                  paddingBottom: i < passengers.length - 1 ? "14px" : "0",
                  borderBottom: i < passengers.length - 1 ? "1px solid #f0f0f0" : "none"
                }}>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "12px" }}>
                    {[
                      ["Name", p.name],
                      ["Seat", p.seatName || p.seat],
                      ["Age", p.age],
                      ["Gender", p.gender]
                    ].map(([label, value]) => (
                      <div key={label}>
                        <div style={{ fontSize: "9.5px", color: "#999", textTransform: "uppercase", fontWeight: "700", marginBottom: "3px" }}>{label}</div>
                        <div style={{ fontSize: "12.5px", fontWeight: "700", color: label === "Seat" ? "#fd561e" : "#1a1a2e", wordBreak: "break-word" }}>
                          {value || "—"}
                        </div>
                      </div>
                    ))}
      ...
      {showRewardConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: "16px" }}>
          <div style={{ background: "white", borderRadius: "20px", padding: "24px", width: "100%", maxWidth: "380px", maxHeight: "90vh", overflowY: "auto" }}>
            {/* Your reward popup content from second version */}
            {/* (I kept it same as your last version for accuracy) */}
            <div style={{ textAlign: "center", marginBottom: "16px" }}>
              <div style={{ fontSize: "36px", marginBottom: "8px" }}>💰</div>
              <h2 style={{ fontSize: "17px", fontWeight: "800", color: "#1a1a2e" }}>Reward Points Summary</h2>
            </div>
            {/* ... rest of reward popup content same as your last code ... */}
            {/* For full length, you can copy the reward popup from your second code block */}
          </div>
        </div>
      )}
